...
class AbstractDiffusion:
    ...
    def prepare_controlnet_tensors(self, refresh:bool=False):
        ...
        for i in range(len(tensors)):
            ...
            if isinstance(control_tensor, dict):
                control_tensor = next(iter(control_tensor.values()))
                
            while control_tensor.dim() < 4:
            ...
            for bboxes in self.batched_bboxes:
                ...
                for bbox in bboxes:
                    ...
                    H, W = control_tensor.shape[-2:]
                    y0, y1 = bbox[1]*opt_f, bbox[3]*opt_f
                    x0, x1 = bbox[0]*opt_f, bbox[2]*opt_f
                ...
                ref_h, ref_w = single_batch_tensors[0].shape[-2:]
                for n, t in enumerate(single_batch_tensors):
                    h, w = t.shape[-2:]
                    if (h, w) != (ref_h, ref_w):
                        single_batch_tensors[n] = F.interpolate(
                            t, size=(ref_h, ref_w), mode="bilinear", align_corners=False
                        )
